...
import {  ArrowLeft, CardSim, Minus, Plus, ShoppingBag, Trash2 } from 'lucide-react';
import { Link } from 'react-router-dom';
import axios from 'axios';

const API_BASE = import.meta.env.VITE_API_BASE_URL;
//we have to create a product where a user can increase or decrease the items in the cart also can see the qty.
 ...
 const handleSubmit = async (e) => {
    ...
    try {
      ...
      await clearCart();
        ...
        // reset the fields after submitting datta
        setName("");
        setEmail("");
        setAddress("");
        setMobile("");
      ...
      <div className={cartPageStyles.emptyCartContainer}>
        <div className={cartPageStyles.emptyCartCard}>
          ...
          <p className={cartPageStyles.emptyCartText}>
              Looks like you haven't added any watches to cart yet
          </p>
                ...
                <div className={cartPageStyles.formContainer}>
                  ...
                  <form onSubmit={handleSubmit} className={cartPageStyles.form}>
                    <div className={cartPageStyles.inputGrid}>
                      <input type="text" value={name} onChange={(e)=>setName(e.target.value)} placeholder='FullName'  className={cartPageStyles.inputBase} required />
                      <input type="email" value={email} onChange={(e)=>setEmail(e.target.value)} placeholder='your@example.com'  className={cartPageStyles.inputBase} required />
                      <input type="text" value={mobile} onChange={handleMobileChange} placeholder='Mobile Number'  className={cartPageStyles.inputBase} required />
                    </div>
                      <textarea value={address} onChange={(e)=>setAddress(e.target.value)} placeholder='Address' className={cartPageStyles.textareaBase} required></textarea>
                      <select value={paymentMethod} onChange={(e)=>setPaymentMethod(e.target.value)} className={cartPageStyles.selectBase} required >
                        <option value="">Select Payment Method</option>
                        <option value="Online">Online</option>
                        <option value="Cash on Delivery">Cash on Delivery</option>
                      </select>
                       <textarea value={note} onChange={(e)=>setNote(e.target.value)} placeholder='Message/Delivery Instructions' className={cartPageStyles.textareaBase} rows={2} required></textarea>
                       <div className={cartPageStyles.formButtonsContainer}>
                          <button type='submit' disabled={submitting} className={cartPageStyles.submitButton}>
